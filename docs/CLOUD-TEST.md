...
2026-09-11 用户要求删除旧测试部署，再试仅预装酒馆助手。已删除并同名重建测试 Worker，
原 D1/R2 和两把密钥保留；当前第三方预装为 Helper 4.9.5，EJS 不加载。
这是单实例试验，不改变仓库默认双插件清单，也不是 Actions 云端部署验收。
范围、验证与更新注意事项见 [酒馆助手单插件云端试验](HELPER-ONLY-CLOUD.md)。
